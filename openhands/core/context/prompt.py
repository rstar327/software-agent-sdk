...
def refine_prompt(prompt: str):
    ...
    On Windows systems, replaces 'terminal' with 'powershell' and 'execute_terminal' with 'execute_powershell'
    ...
    if sys.platform == "win32":
        # Replace 'terminal' with 'powershell' including tool names like 'execute_terminal'
        # First replace 'execute_terminal' with 'execute_powershell' to handle tool names
        result = re.sub(r"\bexecute_terminal\b", "execute_powershell", prompt, flags=re.IGNORECASE)
        # Then replace standalone 'terminal' with 'powershell'
        result = re.sub(r"(?<!execute_)(?<!_)\bterminal\b", "powershell", result, flags=re.IGNORECASE)
        return result
    return prompt
...
class PromptManager:
    ...
    def get_system_message(self, **context) -> TextContent:
        system_message = self.system_template.render(**context).strip()
        return TextContent(text=refine_prompt(system_message))
